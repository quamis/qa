...
class qad {
	...
	*/
	protected function crc32Add($crc32, $str) {
		$this->cache['crc32Add'][$crc32] = $str;
	}
	protected function crc32Exists($crc32) {
		return array_key_exists($crc32, $this->cache['crc32Add']);
	}
	protected function crc32GetStr($crc32) {
		return $this->cache['crc32Add'][$crc32];
	}
	...
	protected function pc_permuteWithPartialSums_faster($string, $perms = "", $depth=0) {
		...
		if (!$string) {
			...
			if( $lperms > $partialCrc32['windowSize']) {
				...
				if($this->crc32Exists($partialCrc32['crc32'][$idx]['crc32'])) {
					$neededSlice = $this->crc32GetStr($partialCrc32['crc32'][$idx]['crc32']);
					$neededSliceFound = true;
					printf("->>> %s\r", $neededSlice);
				}
				else {

					$slice = substr($perms , -$windowSize);
					$crc32 = crc32($slice);
					if($crc32 != $partialCrc32['crc32'][$idx]['crc32']) {
						$crc32ok = false;
					...
					$this->crc32Add($crc32, $slice);

					if(!$crc32ok && !$neededSliceFound) {
						return 1;
					}
				}
			}
...
#file_put_contents("arch.txt", serialize($compressor->compress("\x9c\x8f\xac\xe2\x42\x70\xc9\x7a\x85\x9d\x62\xb5\x0d\x2f\x87\xc1\x55\x68\x5e\x4f\xfe\xd7\xd1\xe1\x3e\xf8\x5a\x42\x29\x75\xf8\x72\x9f\x11\x52\x54\x49\xf0\x00\x7b\x24\x61\xab\xf5\x70\x5f\x04\x3c\x25\x90"))); #
