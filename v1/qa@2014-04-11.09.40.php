	...
	*/
	protected function pc_permuteWithPartialSums_faster($string, $perms = "", $depth=0) {
		...
		if (!$string) {
			...
			if(strlen($perms) > $partialCrc32['windowSize']) {
				...
				$idx = 0;
				while($windowSize<strlen($perms)) {
					$slice = substr($perms2 , -$windowSize);
					if(crc32($slice)!=$partialCrc32['crc32'][$idx]['crc32']) {
						$crc32ok = false;
						$crc32failIndex = $this->desc['len'] - $windowSize;

						if($this->cache[__FUNCTION__]['loops']>5000) {
...
#file_put_contents("arch.txt", serialize($compressor->compress("CbC669xGdsrVxVzewGmtAdgRN"))); # 1.6s
#file_put_contents("arch.txt", serialize($compressor->compress("CbC669xGdsrVxVzewGmtAdgRNPXyHHCh8HG9aGZAvyc73pHSmB"))); # 51s
