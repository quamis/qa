...
class qad {
	...
	protected function gotSumDistribution($arr) {
		...
		if(crc32($data)===$this->desc['ordered']['hashes']['crc32']) {
			...
			if($this->desc['raw']['hints']['partialCrc32']) {
				...
				$this->pc_permuteWithPartialSums($arr, Array(), Array(), $this->desc['raw']['hints']['partialCrc32']);
			}
			else {
	...
	*/
	protected function pc_permuteWithPartialSums($items, $perms = array(), $options = Array(), $partialCrc32) {
		if (empty($items)) { 
			...
			$crc32ok = true;
			$crc32failIndex = 0;
			$slices = array_reverse(str_split($s, $partialCrc32['windowSize']), true);
			...
			if($this->cache[__FUNCTION__]['loops']==2500) {
				...
				$this->log(sprintf("Perm: '%s'[l:%d][fidx:%d][ops:%.1f][ttl:%d]\r", $s, strlen($s), $crc32failIndex, $ops, $this->cache[__FUNCTION__]['perms']));
			...
			for ($i = count($items) - 1; $i >= 0; --$i) {
				 ...
				 $retLvl = $this->pc_permuteWithPartialSums($newitems, $newperms, $options, $partialCrc32);
...
#file_put_contents("arch.txt", serialize($compressor->compress("anaaremerele")));
file_put_contents("arch.txt", serialize($compressor->compress("anaaremereghere")));
