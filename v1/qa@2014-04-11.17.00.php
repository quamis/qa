	...
	public function compress($data, $options = Array()) {
		...
		*/
		...
		while($windowSize<strlen($data)) {
			$a = array();
			$slice = substr($data, -$windowSize);
			$a['crc32'] = crc32($slice);

			$a['order'] = Array();
			$slices = str_split($slice);
			$lch = null;
			foreach($slices as $ch) {
				if($lch>$ch) {
					$a['order'][] = 1;
				}
				else {
		...
		$windowSize = max(12, min($cntArr));
		#var_dump($windowSize);
		$windowSize = 12;	// 4 seems like a magic number, its the fastest in my tests
	...
	public function decompress($desc) {
		...
		if($this->desc['ordered']['hints']['partialSums']) {
			...
			*/
			...
			if(false && $this->desc['ordered']['hints']['partialSums']['sums'][$windowIndex]['dictionary']) {
		...
		if (!$string) {
			...
			if( $lperms > $partialCrc32['windowSize']) {
				...
				if(!$crc32ok) {
					$lch = null;
					#$slices = str_split($slice);
					#foreach($slices as $k=>$ch) {
					for($i=0; $i<strlen($slice); $i++) {
						$ch = $slice[$i];
						if($lch>$ch && $partialCrc32['crc32'][$idx]['order'][$i]==0) {
							return $i;
							#return max(1, $i);
						}

						if($lch<=$ch && $partialCrc32['crc32'][$idx]['order'][$i]==1) {
							return $i;
							#return max(1, $i);
						}
						$lch = $ch;
...
#file_put_contents("arch.txt", serialize($compressor->compress("\x9c\x8f\xac\xe2\x42\x70\xc9\x7a\x85\x9d\x62\xb5\x0d\x2f\x87\xc1\x55\x68\x5e\x4f\xfe\xd7\xd1\xe1\x3e\xf8\x5a\x42\x29\x75\xf8\x72\x9f\x11\x52\x54\x49\xf0\x00\x7b\x24\x61\xab\xf5\x70\x5f\x04\x3c\x25\x90"))); # 66s

#file_put_contents("arch.txt", serialize($compressor->compress(file_get_contents("in01.txt")))); # 4.6s
