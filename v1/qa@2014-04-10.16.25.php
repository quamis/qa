		...
		if (!$string) { 
			...
			while($windowSize<$this->desc['len']) {
				if(crc32(substr($perms, -$windowSize))!=$partialCrc32['crc32'][$idx]['crc32']) {
					#$this->log(sprintf("Slice %s mismatched\n", $perms));
					#print_r($perms);
					$crc32ok = false;
...
#file_put_contents("arch.txt", serialize($compressor->compress("CbC669xGdsrVxVzewGmtAdgRN"))); # 6s
#file_put_contents("arch.txt", serialize($compressor->compress("CbC669xGdsrVxVzewGmtAdgRNPXyHHCh8HG9aGZAvyc73pHSmB")));
